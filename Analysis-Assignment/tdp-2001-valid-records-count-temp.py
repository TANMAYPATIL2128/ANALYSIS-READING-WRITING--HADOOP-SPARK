...
df_AT_BadData=df3.filter(col("Air_Temperature")== 999.9)
#calulating the percentage of bad data to total data
df_AT_Percent= (df_AT_BadData.count()*100.0/df3.count())

#Ebook-chapter5- pg-66 in Schemas
from pyspark.sql import Row
from pyspark.sql.types import StructField,StructType, StringType, LongType, FloatType
Schema = StructType([StructField("BadData_Cnt",StringType(),True),StructField("TotalData_Cnt",StringType(),True),StructField("Percentage",FloatType(),True)])
#first row
Row1 = Row(df_AT_BadData.count(),df3.count(),df_AT_Percent)
mydf=spark.createDataFrame([Row1],Schema)
#writing the file
mydf.write.format("csv").option("header","true").mode("overwrite").save("hdfs://namenode/output/itmd-521/tdp/2001/valid-records-count-temp")
