...
df_AP_BadData=df3.filter(col("Atmospheric_Pressure")== 9999.9)
df_AP_Percent= (df_AP_BadData.count()*100.0/df3.count())
...
#first row
Row1 = Row(df_AP_BadData.count(),df3.count(),df_AP_Percent)
mydf=spark.createDataFrame([Row1],Schema)
#writng the file
mydf.write.format("csv").option("header","true").mode("overwrite").save("hdfs://namenode/output/itmd-521/tdp/2001/valid-records-count-press")
